...
| Membre | Rôle | Sections (HTML + CSS) |
|---|---|---|
| Egbêhitchè Hermione Adorée KPENONHOUN | Chef de groupe & responsable synthèse | Services (`services.css`), Témoignages (`testimonials.css`), Rapport & README |
| Orphéric Géovany SANGNIDJO | Responsable technique | Fondations (`base.css`), Header (`header.css`) |
| John Elie LOKOSSOU | Appui à la mise en forme | À propos (`about.css`), Équipe (`team.css`) |
| Bilal Fernand Pers PEDRO | Rédacteur | Stats (`stats.css`), Footer (`footer.css`) |
| Farel Kant MAHUGNON AHIDEDJI | Coordonnateur | Hero (`hero.css`), Contact (`contact.css`) |
